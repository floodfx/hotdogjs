...
export function deepDiff(oldParts: Tree, newParts: Tree): Tree {
  let diff: Tree = {};
  ...
  for (let i = 0; i < Object.keys(newParts).length; i++) {
    ...
    if (key === "s") {
      ...
      // since both aren't strings, check if they are Parts objects
        ...
        const oldPart = oldParts[key] as Tree;
        const newPart = newParts[key] as Tree;
  ...
  for (let i = 0; i < newArray.length; i++) {
    ...
    // parts are both objects (potentially arrays or not)
      ...
      // both parts are objects
        ...
        const maybeDiff = deepDiff(oldPart as Tree, newPart as Tree);
        // keep if any keys are different
        if (Object.keys(maybeDiff).length > 0) {
          return true;
        }
...
export function diffArrays2<T extends Tree | string>(oldArray: T[], newArray: T[]): T[] {
  const diffArray: any[] = [];
  ...
  for (let i = 0; i < oldArray.length; i++) {
    ...
    // parts are both objects (potentially arrays or not)
      ...
      // both parts are arrays
      if (Array.isArray(newPart) && Array.isArray(oldPart)) {
        const res = diffArrays2<Tree>(oldPart, newPart);
        if (res.length > 0) {
          diffArray.push(...res);
        }
      }
